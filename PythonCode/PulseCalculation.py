...
class TPP:
    ...
    def setGlobalVariables(cls, file_path : str) -> None:
        ...
        cls.b : list[list[float]] = [] # mode vector in [A.U]
        cls.eta : list[float] = [] # lamb dicke parameter
        
        log : str = ""
        
        with open("NormalModeCalculationReport.json", "r") as file:
            data = json.load(file)
            cls.max_k = int(data["N"]) # number of mode. Note that this is different from ADAM's N
            
            for i in range(cls.max_k):
                normal_mode = data[f"{i}_th_mode"]
                cls.omega.append(float(normal_mode["w"]) * 1e6 * np.pi * 2) # convert [MHz] to [rad/s]
                cls.b.append([float(comp) for comp in normal_mode["b"]])
        ...
        with open(file_path, "r") as file:
            ...
            for i in range(cls.max_k):
                cls.eta.append(cls.Deltak * np.sqrt(hbar/(2*M*cls.omega[i])))
        ...
        for i in range(cls.max_k):
            ...
            log += (f"| b : {[round(x,3) for x in cls.b[i]]}".ljust(ALIGN_LENGTH) + '|\n')
...
def theta(k: int, delta : np.array) -> np.array:
    ...
    """
    theta_values = cumtrapz(TPP.omega[k] - delta, TPP.tau_array, initial=0)
    return theta_values
...
def alpha(j : int, k : int, Omega : np.array, delta : np.array) -> complex:
    ...
    #
    alpha_value = ((TPP.eta[k] * TPP.b[k][j] / 2) * 
                   np.trapz(integrand, TPP.tau_array))
    return alpha_value
...
def alphaArray(j : int, k : int, Omega : np.array, 
    ...
    integrand = Omega * np.exp(1j * theta(k, delta))
    #
    alpha_values = ((TPP.eta[k] * TPP.b[k][j] / 2) * 
                   cumtrapz(integrand, TPP.tau_array, initial=0))
    return alpha_values
...
def totalTheta(Omega : np.array, delta : np.array) -> float:
    ...
    result : float = 0
    for k in range(TPP.max_k):
        eta_k : float = TPP.eta[k]
        b_k_j1 : float = TPP.b[k][TPP.target_ion_index1]
        b_k_j2 : float = TPP.b[k][TPP.target_ion_index2]
        theta_k : np.array = theta(k, delta)
        
        integrand_cos = Omega * np.cos(theta(k, delta))
        integrand_sin = Omega * np.sin(theta(k, delta))
        integral_cos = cumtrapz(integrand_cos, TPP.tau_array, initial=0)
        integral_sin = cumtrapz(integrand_sin, TPP.tau_array, initial=0)
...
def totalThetaArray(Omega : np.array, delta : np.array) -> np.array:
     ...
     for k in range(TPP.max_k):
         ...
         result = result + (0.5 * eta_k**2 * b_k_j1 * b_k_j2 * 
                    cumtrapz(integral_cos - integral_sin, 
                             TPP.tau_array, initial=0)
                    )
     
     return result           

def calculateCMatrix() -> np.array:
    C_matrix : list[np.zeros(TPP.N)] =[] # number of mode x number of splited pulse
    
    pulse_interval = TPP.integral_div // TPP.N
    
    for j in range(TPP.max_j):
        for k in range(TPP.max_k):
            alpha_values : np.array = alphaArray(j,k,extendVariable(Omega), extendVariable(delta))
